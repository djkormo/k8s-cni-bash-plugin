...
curl_patch="curl --cacert \"${KUBE_CACERT}\" --request PATCH "${node_resource_path}"  --header 'Content-Type: application/json-patch+json' --header \"Authorization: Bearer ${SERVICEACCOUNT_TOKEN}\"  --data '{"spec":{\"podCIDR\":\"$node_pod_cidr\"}}'"
